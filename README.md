...
- `yarn deploy`: Compiles and deploys the Registry and Forwarder contracts to BNB Smart Chain Testnet, and writes their addresses in `deploy.json`.
...
- `yarn events`: Lists all the `Registered` events from the deployed contract on BNB Smart Chain Testnet.
...
- `REACT_APP_QUICKNODE_URL`: Optional URL to Quicknode for connecting to the BNB Smart Chain Testnet network from the dapp.
...
```js
$ git clone https://github.com/mosinb/defender-metatx-api.git
$ cd defender-metatx-api/
$ yarn
```
...
```js
PRIVATE_KEY="Private key used for signing meta-txs locally (user)"
TEAM_API_KEY="Defender Team API key, used for uploading autotask code"
TEAM_API_SECRET="Defender Team API secret"
BSC_SCAN="bscscan API key if you want to verify contract"
```
...
Create a relayer using [Defender Relay Client](https://docs.openzeppelin.com/defender/relay-api-reference) on BNB Smart Chain Testnet.
...
Use the newly created Relayer to deploy the MinimalForwarder and Registry contracts to BNB Smart Chain Testnet
...
We can then use the script to send the request to our relayer, and [view the transaction on Etherscan](https://testnet.bscscan.com/).  We can also view the name registrations.
...
Create an [Autotask using Defender Client](https://docs.openzeppelin.com/defender/autotasks-api-reference), with a webhook trigger and connected to our BNB Smart Chain Testnet relayer.
...
2. Change to BNB Testnet network in Metamask
